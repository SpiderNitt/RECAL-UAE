...
//    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => WalkThroughApp()));
  ...
  void changeDots(Timer timer) {
    ...
    add = add % 2 == 0
        ? (add % 4 == 0 ? 15 : 13)
        : (add % 3 == 0 ? 17 : (add % 5 == 0 ? 11 : 21));
    String value =
        ((int.parse(dots) + add) > 100 ? 100 : (int.parse(dots) + add))
            .toString();
...
//    }
  ...
  void initState() {
    ...
    _dotTimer =
        new Timer.periodic(const Duration(milliseconds: 750), changeDots);
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      setState(() {
        _dotTimer =
            new Timer.periodic(const Duration(milliseconds: 750), changeDots);
        _navigationTimer =
            new Timer(Duration(milliseconds: 4100), navigationPage);
        dots = "0";
      });
                ...
                Column(
                  ...
                  children: [
                    Center(
                      child: Padding(
                        padding:
                            EdgeInsets.symmetric(vertical: getHeight(10, 1)),
                        child: SpinKitWave(
